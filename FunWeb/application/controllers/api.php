...
class Api extends REST_Controller {
	...
	function student_put(){
		...
		if($this->form_validation->run('student_put')!=false){
			...
			if(!$student_id){
				$this->response(array('status'=>'failure','message'=>'An unexpected error occured while trying to create the student'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
			}
			else{
	...
	function student_post(){
		...
		if (isset($student['student_id'])){
			...
			if($this->form_validation->run('student_post')!=false){
				...
				$update=$this->Model_students->update($student_id, $data);
				if(!$student_id){
					$this->response(array('status'=>'failure','message'=>'An unexpected error occured while trying to update the student'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
				}
				else{
	...
	function student_delete(){
		...
		if (isset($student['student_id'])){
			...
			if(!$deleted){
				$this->response(array('status'=>'failure','message'=>'An unexpected error occured while trying to delete the student'),REST_Controller::HTTP_INTERNAL_SERVER_ERROR);
			}
			else{
